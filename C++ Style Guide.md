...
## 防止重复包含
所有头文件都应该有 #define 保护来防止头文件被多重包含, 命名格式当是: PROJECT_PATH_FILE_H_
...
## 可拷贝和可转移
如果你的类需要就显示定义可拷贝(拷贝构造+拷贝赋值)和可转移(转移构造+转移赋值)函数，否则就把隐式拷贝和转移函数禁用
```
class Copyable {
 public:
...
```
不要为基类提供赋值、拷贝构造、转移构造函数。如果基类需要可复制，提供一个public virtual Clone()函数和protected拷贝构造函数以供派生类实现。
